...
export default function MonitorsView({ lang }: { lang: 'en' | 'es' }) {
    ...
    const toggleMonitor = async (m: MonitorOut) => {
        setUpdating(m.id);
        try {
            const updated = await fetchAuth<MonitorOut>(`/api/monitors/${m.id}`, {
                method: "PUT",
    ...
    if (loading) {
        return <div style={{ padding: '20px', color: 'var(--signal)' }}>{lang === 'es' ? 'CARGANDO MONITORES...' : 'LOADING MONITORS...'}</div>;
    }

    return (
        <div className="view monitors-view" style={{ padding: '0 8px 8px 0' }}>
            <div className="panel" style={{ padding: '24px' }}>
                <h2 className="monitors-title" style={{ color: 'var(--signal)', marginTop: 0, fontFamily: 'var(--mono)', fontSize: '18px' }}>
                    {lang === 'es' ? 'MONITORES DE DETECCIÓN SIEM' : 'SIEM DETECTION MONITORS'}
                </h2>
                <p style={{ color: 'var(--text-dim)', fontSize: '11px', marginBottom: '24px' }}>
                    {lang === 'es'
                        ? 'Reglas y umbrales que filtran alertas Wazuh antes de crear tickets. No son servicios Cowrie/honeypot.'
                        : 'Rules and thresholds that filter Wazuh alerts before ticket creation. Not Cowrie/honeypot services.'}
                </p>

                {error && (
                    <div className="monitors-error" style={{ padding: '12px', marginBottom: '16px', borderRadius: '6px', fontSize: '12px' }}>
                        {lang === 'es' ? 'Error al cargar:' : 'Load error:'} {error}
                        {error.includes('403') && (lang === 'es' ? ' — Solo administradores.' : ' — Admins only.')}
                    </div>
                )}
                ...
                {monitors.length === 0 && !error && (
                    <div style={{ padding: '24px', textAlign: 'center', color: 'var(--text-dim)', fontSize: '13px' }}>
                        {lang === 'es'
                            ? 'No hay monitores en la base de datos. Reinicie el backend para crear los predeterminados.'
                            : 'No monitors in database. Restart the backend to seed defaults.'}
                    </div>
                )}

                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(450px, 1fr))', gap: '20px' }}>
                    {monitors.map(m => (
                        <div key={m.id} className="monitor-card" style={{
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                                ...
                                <div
                                    ...
                                    style={{
                                        ...
                                        borderRadius: '11px', cursor: updating === m.id ? 'wait' : 'pointer', position: 'relative', transition: '0.3s',
                                    }}
                                >
                                    <div style={{
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginTop: '20px' }}>
                                <div style={{ flex: 1 }}>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => updateThreshold(m, parseInt(e.target.value, 10))}
